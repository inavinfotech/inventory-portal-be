...
async def get_low_stock():
    async with db_helper.get_db_connection() as db:
        async with db.execute("""
            SELECT p.id, v.id as variant_id, p.name, COALESCE(v.sku, p.sku) as sku, v.weight, i.quantity, i.low_stock_threshold
            FROM inventory i
            ...
            return [
                {
                    ...
                    "sku": r[3],
                    "weight": r[4],
                    "current_quantity": r[5],
                    "threshold": r[6]
                } for r in rows
            ]

@router.get("/{product_id}", response_model=StockResponse)
async def get_stock(product_id: int = Path(..., gt=0), variant_id: Optional[int] = Query(None)):
...
async def bulk_add(batch: BulkUpdate):
    try:
        updates = [{"product_id": i.product_id, "variant_id": i.variant_id, "quantity": i.quantity, "reference_id": i.reference_id} for i in batch.updates if i.type == "IN"]
        if not updates:
...
async def bulk_remove(batch: BulkUpdate):
    try:
        updates = [{"product_id": i.product_id, "variant_id": i.variant_id, "quantity": i.quantity, "reference_id": i.reference_id} for i in batch.updates if i.type == "OUT"]
        if not updates:
