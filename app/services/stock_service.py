...
class StockService:
    ...
    @staticmethod
    async def get_stock(product_id: str, variant_id: Optional[str] = None) -> int:
        async with db_helper.get_db_connection() as db:
            if variant_id:
                async with db.execute(
                    "SELECT quantity FROM inventory WHERE variant_id = ?", (variant_id,)
                ) as cursor:
    ...
    @staticmethod
    async def _ensure_product_exists(db: aiosqlite.Connection, product_id: str, variant_id: Optional[str] = None):
        ...
        if variant_id:
            async with db.execute(
                "SELECT id FROM product_variants WHERE id = ? AND product_id = ?", (variant_id, product_id)
            ) as cursor:
                if not await cursor.fetchone():
                    raise ValueError(f"Variant with id {variant_id} does not exist for product {product_id}")

    @staticmethod
    async def _init_inventory_if_missing(db: aiosqlite.Connection, product_id: str, variant_id: Optional[str] = None):
        if variant_id:
            async with db.execute(
                "SELECT id FROM inventory WHERE variant_id = ?", (variant_id,)
            ) as cursor:
                if not await cursor.fetchone():
                    await db.execute(
                        "INSERT INTO inventory (id, product_id, variant_id, quantity) VALUES (?, ?, ?, 0)",
                        (str(uuid.uuid4()), product_id, variant_id)
                    )
        else:
            async with db.execute(
                "SELECT id FROM inventory WHERE product_id = ? AND variant_id IS NULL", (product_id,)
            ) as cursor:
                if not await cursor.fetchone():
                    await db.execute(
                        "INSERT INTO inventory (id, product_id, variant_id, quantity) VALUES (?, ?, NULL, 0)",
    ...
    async def _check_idempotency(
        ...
        params: list = [product_id, reference_id]
        if variant_id:
    ...
    @staticmethod
    async def add_stock(product_id: str, qty: int, reference_id: Optional[str] = None, variant_id: Optional[str] = None) -> int:
        if qty <= 0:
        ...
        async with db_helper.get_db_connection() as db:
            ...
            try:
                ...
                await db.execute(
                    "INSERT INTO stock_movements (id, product_id, variant_id, quantity, type, reference_id) VALUES (?, ?, ?, ?, 'IN', ?)",
                    (str(uuid.uuid4()), product_id, variant_id, qty, reference_id)
                )
                await db.commit()
                return await StockService.get_stock(product_id, variant_id)
    ...
    @staticmethod
    async def remove_stock(product_id: str, qty: int, reference_id: Optional[str] = None, variant_id: Optional[str] = None) -> int:
        if qty <= 0:
            raise ValueError("Quantity must be positive")
    ...
    @staticmethod
    async def adjust_stock(product_id: str, new_qty: int, reference_id: Optional[str] = None, variant_id: Optional[str] = None) -> int:
        if new_qty < 0:
        ...
        async with db_helper.get_db_connection() as db:
            ...
            try:
                ...
                params: list = [new_qty, product_id]
                if variant_id:
                    query += " AND variant_id = ?"
                    params.append(variant_id)
                else:
                ...
                await db.execute(query, params)
                await db.execute(
                    "INSERT INTO stock_movements (id, product_id, variant_id, quantity, type, reference_id) VALUES (?, ?, ?, ?, 'ADJUST', ?)",
                    (str(uuid.uuid4()), product_id, variant_id, new_qty, reference_id)
                )
                await db.commit()
                return new_qty
            except Exception as e:
        ...
        async with db_helper.get_db_connection() as db:
            ...
            try:
                ...
                for update in updates:
                    ...
                    variant_id: Optional[str] = update.get("variant_id")
